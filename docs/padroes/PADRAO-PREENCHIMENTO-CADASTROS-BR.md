...
- A correção automática pode ocorrer durante digitação, colagem e substituição quando a implementação preservar cursor, seleção e composição do teclado; caso contrário, deve ocorrer em momento estável, como ao sair do campo. Em ambos os casos, o valor efetivamente salvo deve ser normalizado novamente antes da gravação.
