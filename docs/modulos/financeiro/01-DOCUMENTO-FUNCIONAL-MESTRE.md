...
### FASE 10 — Integração visual incremental

Caixa, Estornos, repasses, fiscal interno e painéis da proprietária/médico foram integrados localmente. A interface usa as RPCs oficiais, leitura sob RLS, valores em centavos e teste de navegador com HTTP sintético; isso não equivale a E2E autenticado remoto. Existe uma divergência entre a decisão aprovada (proprietária e recepção podem solicitar estorno) e a FASE 5 aplicada (`financeiro_solicitar_estorno` restringe a solicitação à recepção). A interface não oferece essa ação à proprietária enquanto o banco não for corrigido por migration aditiva, sem alterar a migration homologada. A revisão pela proprietária permanece disponível. Repasses são confirmados somente após PIX/transferência externa, sem movimento de caixa criado pela UI. O fiscal registra apenas solicitações internas, sem alegar emissão ou cancelamento externo. Detalhes nos checkpoints 35–37 e nas seções 16–18 do documento 11.
...
### FASE 14 — Conceito visual aprovado (23/09/2026)
