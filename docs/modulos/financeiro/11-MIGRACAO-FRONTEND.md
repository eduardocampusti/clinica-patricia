...
Validação: 11 testes unitários/contratuais, 14 testes Playwright com HTTP sintético interceptado em desktop/mobile, build e lint aprovados. O teste de navegador cobre legado, médico sem acesso, abertura, leitura do resumo, suprimento, sangria em três etapas, fechamento divergente justificado, aprovação, devolução com orientação e segunda tentativa. Capturas `scratch/financeiro-caixa-{desktop,mobile}.png` foram revisadas; o contraste do botão principal foi corrigido. Nenhum teste autenticado contra o Supabase real foi executado nesta etapa; portanto, a migração integral da FASE 10 não está homologada. Próximos trabalhos independentes: concluir as interfaces de estornos, repasses, fiscal interno, dashboards e relatórios, depois validar E2E e planejar a desativação do legado conforme as dependências da FASE 11.

## 16. FASE 10 — Estornos e dashboards (integração local posterior)

`src/pages/FinanceiroModulo.tsx` organiza o Financeiro por papel: proprietária vê Caixa/Estornos/Painel, recepção Caixa/Estornos, médico somente seu Painel. O Financeiro antigo continua presente no código, mas a rota atual utiliza o módulo novo. Não há entrada de UUID técnico.

Estornos: `src/lib/financeiro/financeiro.estornos-leitura.ts` lê recebimentos e componentes sob RLS, nomes para operação, estornos anteriores e fila de solicitações. A disponibilidade visual desconta estornos não rejeitados por forma; o banco revalida na RPC. A recepção solicita estorno total/parcial pelas formas originais, com motivo e idempotência; a proprietária revisa/aprova/rejeita com confirmação. A ação de **solicitar pela proprietária** está intencionalmente indisponível: a FASE 5 aplicada aceita apenas `recepcao` em `financeiro_solicitar_estorno`, em desacordo com a matriz aprovada. A futura correção deverá ser aditiva e testada, sem editar a migration aplicada. Aprovação continua sendo atribuição da proprietária.

Painéis: `src/pages/FinanceiroPainel.tsx` consome apenas `financeiro_dashboard_proprietaria` e `financeiro_dashboard_profissional`. O wrapper médico não recebe identificador de profissional nem papel selecionável. Filtros de data usam dias locais de Bahia, convertidos a `[início, próximo dia)` inclusive com offset histórico; todas as clínicas apenas dentro dos vínculos autorizados. Indicadores deixam explícito que coorte líquida pode incorporar estornos posteriores, eventos de estorno usam data de efetivação, e estoques são posição atual. Proprietária vê fiscal/caixa/alertas, médico apenas suas métricas e repasses. Valores monetários são formatados dos decimais bancários em centavos, nunca recomputados por ponto flutuante.

Validação local: 13 testes unitários/contratuais, seis cenários Playwright de Estornos e quatro de Painel em desktop/mobile, build e lint aprovados. Testes de navegador interceptam o HTTP e não provam sessão real, RLS remoto ou E2E autenticado. Capturas locais em `scratch/` foram inspecionadas. Nenhuma alteração remota ocorreu neste incremento. Os próximos blocos da FASE 10 são repasses, fiscal interno, relatórios, correção controlada do papel de solicitação de estorno e validação integrada real antes de qualquer desativação do legado.

## 17. FASE 10 — Repasses (integração local posterior)

Proprietária acessa a aba Repasses no módulo; recepção não a vê. A leitura de `repasses`, `repasses_itens`, aplicações, recebimentos, pacientes e profissionais respeita RLS. Filtro por status e paginação não dependem de UUID digitado. A revisão mostra bruto profissional, estornos anteriores ao pagamento, ajustes aplicados, líquido oficial e itens quitados. Composição além do limite de tela bloqueia a confirmação visual e exige o relatório paginado. Médico continua vendo apenas seus repasses no dashboard próprio.

Após PIX ou transferência externa, a proprietária informa meio, referência do comprovante e observação opcional; o frontend chama `financeiro_confirmar_repasse` com a mesma chave idempotente em retry. A RPC mantém autoridade sobre papel, valor e transição terminal. A interface não calcula novo valor, não cria movimento de caixa e não executa pagamento externo. Teste Playwright 2/2 em desktop/mobile com HTTP sintético interceptado; build/typecheck aprovado. E2E autenticado remoto permanece pendente. Nenhuma alteração no Supabase nesta etapa.

## 18. FASE 10 — Fiscal interno (integração local posterior)

Proprietária e recepção acessam a aba Fiscal, com leitura sob RLS, filtro por estado e paginação. `FinanceiroFiscal` mostra paciente, status e atualização; solicita emissão apenas em `pendente`/`erro_emissao`, cancelamento com motivo apenas em `emitida`/`erro_cancelamento`. Os wrappers `financeiro_solicitar_emissao_fiscal` e `financeiro_solicitar_cancelamento_fiscal` conservam chave idempotente nas tentativas. Estados e permissões finais são validados pelo banco.

A UI avisa expressamente que a solicitação interna **não** emite/cancela nota no provedor externo. Nenhuma credencial, API, certificado, callback ou emissão real foi acrescentada. Playwright sintético 2/2 em desktop/mobile testou pedido de emissão e aviso; build/typecheck, lint e 13 testes contratuais passaram. O fluxo de cancelamento ainda não recebeu teste de navegador específico. E2E autenticado remoto segue pendente; não houve alteração no Supabase.

## 19. FASE 10D — Estornos homologados

A decisão funcional posterior substitui a divergência histórica descrita na seção 16: somente a recepção solicita estorno; somente a proprietária revisa e aprova/rejeita, com efetivação pela RPC. `financeiro_solicitar_estorno` não foi alterada. A lista mostra paciente, profissional, data, valor e componentes originais no contexto da clínica selecionada; filtro local da página e preenchimento do total disponível são apoio operacional. O saldo visual não substitui a validação bancária.

O roteiro `database/tests/financeiro/20260922_fase10d_estornos.sql` passou no projeto remoto `xftnkusbyqzyvzrovroj` dentro de `BEGIN`/`ROLLBACK`: split, parcial, total, segundo estorno, dinheiro/PIX/cartão, reserva por forma, excedente, rejeição, caixa fechado, caixa legado, médico, recepção, proprietária, outra clínica, idempotência e conflito. Antes/depois: recebimentos 0, componentes 0, estornos 0, repasses 0, fiscal 0, movimentos 0, sessões 1. Nenhuma fixture persistiu. Testes Playwright da UI usam HTTP sintético; smoke visual autenticado real permanece pendente.

## 20. FASE 10E — Repasses homologados

`FinanceiroRepasses` mantém leitura sob RLS e confirmação exclusivamente após PIX/transferência externa, sem recalcular nem transferir valor. O painel médico passou a explicitar os campos oficiais `valor_bruto_profissional`, `valor_estornos_antes_pagamento`, `valor_ajustes_aplicados`, `valor_liquido` e `status` de sua própria lista; não envia `profissional_id` ao banco.

O roteiro `database/tests/financeiro/20260922_fase10e_repasses.sql` passou remotamente em `BEGIN`/`ROLLBACK`: geração atômica, RLS por papel/clínica, confirmação externa idempotente e sem movimento de caixa, estorno antes/depois do pagamento, ajuste compensado em repasses futuros e bloqueio de líquido negativo. Contagens persistentes finais: recebimentos/estornos/repasses/ajustes/movimentos 0, sessões 1. Painel 4/4 e Repasses 2/2 em Playwright desktop/mobile, 13/13 testes financeiros, build e lint passaram. As telas de navegador usam HTTP sintético, portanto smoke autenticado remoto ainda não está comprovado. Nenhuma migration ou dado persistente foi criado nesta subfase.

## 21. FASE 10F — Painéis por papel
...
## 23. FASE 10H — Relatórios na interface

`FinanceiroRelatorios` integra as RPCs aplicadas da FASE 9 para proprietária e médico. A proprietária pode gerar consolidado, recebimentos, repasses e fiscal, respeitando período `[início,fim)`, clínica autorizada, pessoa, forma e estados nos domínios definidos. O médico exporta somente recebimentos e repasses próprios; a interface e os wrappers não recebem `profissional_id` para esse papel. Filtros por pessoa são seleções de nomes sob RLS e os IDs permanecem internos.

A solicitação de exportação é auditada antes da coleta. Os detalhes usam paginação keyset, contexto, marcador anti-drift, totais estáveis, revalidação final, limite de 50 mil registros e reconciliação em centavos inteiros. Cancelamento interrompe chamadas paginadas por `AbortSignal`; nenhum arquivo é baixado depois do cancelamento. O dashboard é usado apenas para métricas do consolidado, sem substituir os totais reconciliados dos datasets.
