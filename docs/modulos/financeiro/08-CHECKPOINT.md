...
## 43. FASE 10H — Relatórios na interface

**Estado:** interface local integrada e validação automatizada sintética aprovada; smoke visual autenticado real pendente.

- `FinanceiroRelatorios` foi adicionado por lazy loading ao módulo para proprietária e médico. A proprietária escolhe consolidado, recebimentos, repasses ou fiscal, com período, clínica e filtros autorizados. O médico recebe somente painel e relatórios do próprio universo; nenhuma chamada médica envia `profissional_id`.
- A interface registra a solicitação de exportação antes da coleta, pagina os datasets da FASE 9 por cursor, compara contexto/marcador/totais, revalida a população e reconcilia os campos aditivos em centavos antes de gerar PDF/XLSX. Cancelamento usa `AbortSignal` e não produz arquivo após ser solicitado.
- A apresentação utiliza allowlist de colunas. UUID, CPF, payload fiscal e campos extras não entram no arquivo. Formas de pagamento estruturadas são convertidas apenas para texto legível por uma allowlist de dinheiro/PIX/cartão, sem recalcular snapshots financeiros. Texto iniciado por fórmula permanece texto no XLSX.
- Playwright sintético: 8/8 cenários funcionais em desktop/mobile (o processo do servidor Vite precisou ser interrompido depois de os oito testes passarem por retenção de teardown no Windows). Cobertura: múltiplas páginas, revalidação, anti-drift, filtros sem nomes na auditoria, PDF, XLSX vazio do médico e ausência de `profissional_id`. Quatro screenshots foram inspecionados.
- `npm run test:financeiro`: 15/15; `npm run build`: aprovado; `npm run lint`: aprovado com o warning preexistente de Fast Refresh. A tela própria gerou chunk de 23,76 kB gzip 6,93 kB; geradores ficaram no chunk sob demanda de 501,15 kB gzip 157,64 kB. O chunk principal permaneceu 624,30 kB, pendência geral para a auditoria final de bundle.
- Nenhuma migration, SQL remoto, fixture persistente, `supabase db push` ou Git push nesta subfase. A infraestrutura aplicada da FASE 9 permaneceu imutável. Próximo passo: auditoria do legado, teste integrado final e documentação de encerramento.

## 44. Teste integrado final e encerramento do legado

**FASE 10 — MIGRAÇÃO FRONTEND CONCLUÍDA.** Smoke visual autenticado real permanece como validação externa pendente por ausência de sessão segura; não foram solicitadas senhas nem criadas credenciais. Integração fiscal externa permanece como evolução futura dependente de provedor/API. Nenhuma dessas pendências reabre ou bloqueia a migração concluída.

- `database/tests/financeiro/20260923_fase10_integrado_final.sql` passou no projeto `xftnkusbyqzyvzrovroj` em uma única transação com `ROLLBACK`. A primeira execução chegou ao fim funcional, mas uma asserção de contagem incluía o movimento de estorno junto do movimento principal; a transação abortou e reverteu. A asserção foi restringida ao tipo `recebimento` e a reexecução passou.
- Fluxo validado: agendamento confirmado → recebimento R$ 500,00 em dinheiro/PIX/cartão → solicitação fiscal interna → estorno de R$ 50,00 em dinheiro → resumo de caixa R$ 250,00 → fechamento → repasse líquido R$ 360,00 → confirmação externa → dashboards → relatórios de recebimentos/repasses/fiscal → auditoria de exportação.
- Cobertura negativa: proprietária não solicita estorno, recepção não aprova fechamento nem confirma repasse, médico vê somente o próprio universo, outra clínica é negada, `anon` não executa as RPCs, retries são idempotentes e transições fiscais inválidas falham.
- Antes/depois: recebimentos 0, pagamentos 0, estornos 0, repasses 0, documentos fiscais 0, movimentos 0, sangrias 0, fechamentos 0 e sessões 1. Migration remota final permaneceu `20260922181438_financeiro_fase10c_resumo_caixa`. Caixa legado: aberto, R$ 150,50 de abertura, duas entradas, R$ 1.000,00.
