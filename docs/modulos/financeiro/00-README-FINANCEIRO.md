...
**Status do produto:** FASES 1–9 aplicadas e homologadas; FASES 10A/10B concluídas e aprovadas pelo usuário. **FASE 10 — Migração frontend: EM EXECUÇÃO.** FASE 10C — Caixa: **HOMOLOGAÇÃO TÉCNICA APROVADA** no projeto `xftnkusbyqzyvzrovroj`, com RPC de leitura aplicada, validação transacional real e `ROLLBACK`; smoke visual autenticado ainda **PENDENTE**. FASE 10D — Estornos: separação vigente de funções (recepção solicita; proprietária decide) e homologação transacional com `ROLLBACK`. FASE 10E — Repasses: geração, confirmação externa, permissões e ajustes homologados em transação com `ROLLBACK`. FASE 10F — Painéis por papel homologados tecnicamente; Dashboard inicial sem números financeiros fictícios. FASE 10G — Fiscal interno homologado em transação com `ROLLBACK`, sem integração externa. FASE 10H — Relatórios: interface de proprietária/médico integrada localmente com filtros, paginação, anti-drift, reconciliação, PDF/XLSX, cancelamento e auditoria da solicitação; testes sintéticos aprovados. E2E visual autenticado e integração fiscal externa permanecem pendentes. Evidências em `11-MIGRACAO-FRONTEND.md` e `08-CHECKPOINT.md`.
