...
2. A configuração cria PR de versão em rascunho, para revisão humana, com `initial-version: 0.1.0` e regras pré-1.0 explícitas. Release Please 17.11.2 foi exercitado localmente **nas funções de cálculo**, sem escrita remota: na ausência de release anterior, a primeira versão calculada foi `0.1.0`; partindo de `0.1.0`, `fix` resultou em `0.1.1`, `feat` em `0.2.0` e breaking pré-1.0 em `0.2.0`. A simulação integral da branch integrada ainda deve confirmar o resultado antes do merge.
